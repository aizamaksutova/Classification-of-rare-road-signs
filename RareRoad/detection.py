...
class FacesDataset(Dataset):
    def __init__(self, train_gt, train_img_dir, img_size, transform=None):
        ...
        for img_filename, img_points in train_gt.items():
            ...
            self.pair_points.append(list(zip(img_points[0::2], img_points[1::2])))
    ...
    def __getitem__(self, index):
        ...
        result = augs(image=np.array(img), keypoints=self.pair_points[index])
        img, img_points = result["image"], np.array(result["keypoints"]).reshape(-1)
        if self._transform:
...
class FacesModel(nn.Module):
    def __init__(self, img_size):
        ...
        self.seq_tail = nn.Sequential(
            nn.Linear(12 * 12 * 256, 64),
            nn.ReLU(),
            nn.Linear(64, 28),
        )
    ...
    def forward(self, x):
        x = self.seq_base(x)
        x = self.seq_tail(x.view(x.size(0), -1))
        return x
